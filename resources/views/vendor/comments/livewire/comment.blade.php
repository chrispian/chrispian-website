...
>
                ...
                <ul class="comments-comment-header-actions">
                    ...
                    @if($writable)
                        @can('update', $comment)
                            <li>
                                @if ($isEditing)
                                    <a href="#" id="stop" wire:click.prevent="stopEditing" aria-role="button">{{__('comments::comments.cancel')}}</a>
                                @else
                                    <a href="#" id="start" wire:click.prevent="startEditing" aria-role="button">{{__('comments::comments.edit')}}</a>
                                @endif
                            </li>
                        @endcan
                        @can('delete', $comment)
                                    ...
                                    @can('react', $comment)
                                        ...
                                        <x-comments::modal x-show="open" compact left>
                                            <div class="comments-reaction-picker-reactions">
                                                @foreach(Config::allowedReactions() as $reaction)
                                                    @php
    ...
    @if($showReplies && $comment->isTopLevel())
        <div class="comments-nested">
            @if($this->newestFirst)
                @if(auth()->check() || Config::allowAnonymousComments())
